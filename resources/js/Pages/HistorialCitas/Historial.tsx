...
const Historial = () => {
    const { historialC } = usePage<HistorialPageProps>().props;

    const { search, status, date, isLoading: isFiltersLoading, handleFilterChange } = useFilters("historial");
                    ...
                    <div className="p-6">
                                                    ...
                                                    <tr key={cita.id} className="border-b">
                                                        <td className="px-4 py-3">
                                                            ...
                                                            <div className="text-sm text-gray-500 flex items-center gap-1">
                                                                ...
                                                                <span className="font-medium text-gray-500">{cita.users.phone}</span>
                                                            </div>
                                                            <div className="text-sm text-gray-500 flex items-center gap-1">
                                                                <Phone className="w-4 h-4 text-green-500" />
                                                                <span className="font-medium text-gray-500">{cita.users.email}</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-3">
                                                            <div className="flex items-center gap-2">
                        ...
                        <RoleGuard allowedRoles={['Patient']}>
                            <div className="relative border-l border-gray-200">
                                {historialC.data.map((cita, index) => (
                                    <div key={cita.id} className="mb-10 ml-6">
                                        {/* Icono principal en la línea de tiempo */}
                                        <div
                                            className={`absolute w-8 h-8 -left-4 rounded-full ${cita.status === 'Confirmada'
